...
interface ICollateralEscrow {
    ...
    event Seized(uint256 indexed loanId, uint256 seized, address indexed to);
    ...
    /// @notice Return all collateral of `loanId` to `to` (repaid, cancelled, or remainder after default).
    /// Loan registry only.
    /// @return amount Collateral released.
    function release(uint256 loanId, address to) external returns (uint256 amount);

    /// @notice Default path: send up to `loss` to `to`. Any excess stays in escrow for the borrower to withdraw
    /// through the loan registry (which checks sanctions). Loss waterfall only.
    /// @return seized Amount sent to `to` (= min(loss, collateral)).
    function seize(uint256 loanId, uint256 loss, address to) external returns (uint256 seized);
